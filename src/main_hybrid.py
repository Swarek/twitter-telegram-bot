#!/usr/bin/env python3
"""Bot Twitter-Telegram avec scraper hybride (RSS + fallbacks)"""
import asyncio
import signal
import sys
import os
...
from scraper.rss_scraper import HybridScraper
from publisher import TelegramPublisher
from models import db

logger = get_logger(__name__)

class TwitterTelegramBotHybrid:
    def __init__(self):
    ...
    async def setup(self):
        ...
        logger.info("Initialisation du bot Twitter-Telegram (Version Hybride)...")
        ...
        logger.info("✅ Bot initialisé avec succès (Mode Hybride)")
    ...
    async def process_account(self, account: Dict):
        ...
        try:
            ...
            logger.info(f"{len(tweets)} tweets trouvés pour @{username} (via {self.scraper.last_method})")
...
def signal_handler(bot: TwitterTelegramBotHybrid):
...
if __name__ == "__main__":
    ...
    logger.info("=" * 50)
    logger.info("Twitter-Telegram Bot v1.0.0 - Version Hybride")
    logger.info(f"Environnement: {settings.environment}")
    logger.info(f"Log level: {settings.log_level}")
    logger.info(f"Canal Telegram: {settings.telegram_channel_id}")
    logger.info("Méthodes de scraping: RSS, Démo")
    logger.info("=" * 50)
